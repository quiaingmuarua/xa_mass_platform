...
`pending-runner-audit` until unchanged live Host/Server processes, 90 control
...
- Two Groups of 50 Workers, one WebSocket Adapter and one Manager per Group.
- String: 20 stable A, 20 stable B and 10 mutable targets initially A.
- Phone: 50 cross-Group controls. Every Worker starts with an empty-string sentinel.
...
- Three background Tasks: String A, String B and unrestricted Phone. Each has
  5,000 Items, 1,000 ms Handler delay, priority 50 and at most 50 candidates.
- Four witness Tasks each have 10 Items, 100 ms delay, priority 10 and at most
  10 candidates. All Tasks use the public finite PRECOMPUTED lifecycle with
  maxRetryTimes=3. Total: seven Tasks and 15,040 submitted Items.
...
| A to B | Four Lab writers issue eight PATCH rounds over ten targets, sequential per Worker and at least 500 ms between rounds, without remote observation waits. B witness subsequently succeeds on actual targets. |
...
and Platform maps, exact live Lab identity/run state and all 100 connections.

Actual executors of witness Items must be among the ten targets. Stable A/B
