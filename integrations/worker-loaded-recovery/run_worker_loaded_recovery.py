#!/usr/bin/env python3
"""One-shot Linux owner for Worker Loaded Capacity + Recovery Stability."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run Worker Loaded Capacity + Recovery Stability."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=(
            REPOSITORY_ROOT / "build/worker-loaded-recovery-proof"
        ),
    )
    parser.add_argument(
    ...
    if not sys.platform.startswith("linux"):
        parser.error(
            "Worker Loaded Capacity + Recovery Stability requires "
            "Linux /proc resource evidence"
        )
    ...
    try:
        ...
        timeline = evidence_root / "worker-loaded-recovery-timeline.jsonl"
        ...
        _write_json(
            evidence_root / "worker-loaded-recovery-summary.json",
            {
                "proofId": proof_id,
        ...
        print(
            "Worker Loaded Capacity + Recovery Stability passed "
...
def _start_server(
    ...
    jars = sorted(
        path
        for path in (REPOSITORY_ROOT / "server_boot_jvm/build/libs").glob(
...
def _start_stage(
    ...
    executable = (
        REPOSITORY_ROOT
        / "integrations/worker-loaded-recovery/build/install/"
        "xa-mass-worker-loaded-recovery/bin/xa-mass-worker-loaded-recovery"
    )
    if os.name == "nt":
    ...
    log_path = output_root / f"loaded-recovery-harness-{stage}.log"
...
def _wait_stage(
    ...
    if result != 0:
        raise RuntimeError(
            f"loaded recovery Java stage exited with {result}; log:\n{_tail(log_path)}"
        )
...
def _wait_gate(
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.05)
    raise RuntimeError(f"timed out waiting for loaded recovery gate {path.name}")
...
def _validate_gate(
    ...
    if set(value) != expected_fields:
        raise RuntimeError("loaded recovery process gate fields changed")
    if (
        value.get("proofId") != proof_id
        or value.get("stage") != stage
        or value.get("checkpoint") != checkpoint
    ):
        raise RuntimeError("loaded recovery process gate identity changed")
    if not isinstance(value.get("atEpochMillis"), int):
        raise RuntimeError("loaded recovery process gate timestamp is invalid")
    ...
    def __init__(
        ...
        *,
        ...
        self._thread = threading.Thread(
            target=self._run,
            name="worker-loaded-recovery-sampler",
            daemon=True,
        )
...
def _validate_stage_summary(
    ...
    for field, value in expected.items():
        if summary.get(field) != value:
            raise RuntimeError(
                f"loaded recovery stage summary {field} must be {value}, "
                f"observed {summary.get(field)!r}"
            )
    tasks = summary.get("tasks")
    if not isinstance(tasks, list) or len(tasks) != TASKS_PER_STAGE:
        raise RuntimeError("loaded recovery stage must report ten Tasks")
    for task in tasks:
        if (
            ...
            raise RuntimeError("loaded recovery stage Task export is incomplete")
    if (
        summary.get("minimumConnectedDuringWork", 0)
        < options.minimum_retained_converged
    ):
        raise RuntimeError("loaded recovery stage lost the minimum workload connections")
    if (
        summary.get("postWorkConnectedAndHot", 0)
        < options.minimum_retained_converged
    ):
        raise RuntimeError("loaded recovery stage did not reconverge after workload drain")
    ...
    if (
        ...
        raise RuntimeError("loaded recovery stage missed the mutation success window")
    if (
        checkpoint_unresolved != total_items - checkpoint_succeeded
        or checkpoint_unresolved < total_items - total_items // 2
    ):
        raise RuntimeError("loaded recovery stage missed the mutation backlog window")
    ...
    if (
        ...
        raise RuntimeError("loaded recovery stage did not establish initial headroom")
    if stage == "initial-contraction":
    ...
    if summary.get("batchStopRequestCount") != expected_stop_batches:
        raise RuntimeError("loaded recovery stage batch-stop count changed")
