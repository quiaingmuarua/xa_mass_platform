...
| `chaos smoke bundle (CI gate)` | `scripts/run-chaos-smokes.sh` | fast CI gate running six runtime/aggregate/trace-first probes; exits non-zero if any probe fails; wired into `.github/workflows/maven.yml` `chaos-smokes` job | `target/chaos-reports/` |
