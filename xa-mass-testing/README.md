...
| `polling scheduling fast soak` | `scripts/run-polling-scheduling-fast-soak.sh` | 20-30 second scheduled/manual polling soak profile with mixed results, late worker join, result sequential read, and canonical trace validation; not a PR gate | `target/soak-reports/`, `target/soak-traces/` |
