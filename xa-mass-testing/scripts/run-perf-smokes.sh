...
if [[ -n "${MASS_PERF_TASK_FLOW_BACKENDS:-}" ]]; then
  ...
  for backend in "${TASK_FLOW_BACKENDS[@]}"; do
    ...
    java "$@" \
      ...
      "-Dmass.load.expireFirstAttemptEveryNth=${MASS_PERF_TASK_FLOW_EXPIRE_FIRST_ATTEMPT_EVERY_NTH:-0}" \
      "-Dmass.load.staleResultEveryNth=${MASS_PERF_TASK_FLOW_STALE_RESULT_EVERY_NTH:-0}" \
