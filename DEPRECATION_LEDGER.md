...
| --- | --- | --- | --- | --- |
| `com.xa.mass.gateway.model.massMessage.MassMessage` | `xa-mass-gateway`, `xa-mass-sdk`, `xa-mass-dev-app`, gateway/mock tests, docs | transport-neutral dispatch/result/system-event contracts plus adapter-local frame DTO only for the current WebSocket path | 174 repo references | remove after WebSocket task/control bridges, mock workers, and related tests no longer require the compatibility frame shell |
| `com.xa.mass.gateway.model.massMessage.MessageContext` | `xa-mass-gateway`, `xa-mass-sdk`, `xa-mass-dev-app`, gateway/mock tests, docs | canonical task/attempt/event data in runtime models plus adapter-local endpoint metadata only where the WebSocket path still needs it | 56 repo references | remove together with `MassMessage` shell cleanup after WebSocket frame context is no longer required outside adapter-local code |
