...
class RecordingDeliverSeedQueue(DeliverSeedQueue):
    def __init__(self, *, fail_on: str | None = None) -> None:
        ...
        self.attempts: list[tuple[str, tuple[DeliverSeed, ...]]] = []
        self.appended: list[tuple[str, tuple[DeliverSeed, ...]]] = []
    ...
    def append_deliver_seeds(
        ...
        *,
        ...
        batch = (endpoint_manager_id, tuple(deliver_seeds))
        self.attempts.append(batch)
        if endpoint_manager_id == self.fail_on:
...
class TaskItemDispatchPacerTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.dispatch_runtime = Mock(spec=TaskDispatchRuntime)
    ...
    def test_public_contract_and_config_validation(self) -> None:
        ...
        self.assertEqual(
            ...
            list(inspect.signature(DeliverSeedQueue.append_deliver_seeds).parameters),
        )
        self.assertIs(py_example.TaskItemDispatchPacer, TaskItemDispatchPacer)
    ...
    def test_exhausted_missing_and_stale_items_do_not_produce_seeds(self) -> None:
        ...
        seed = self.queue.appended[0][1][0]
    ...
    def test_whole_round_aggregates_by_endpoint_manager(self) -> None:
        ...
        self.assertEqual(1, len(self.queue.appended))
        endpoint_manager_id, seeds = self.queue.appended[0]
        self.assertEqual("endpoint-1", endpoint_manager_id)
    ...
    def test_different_endpoint_managers_are_appended_separately(self) -> None:
        ...
        self.assertEqual(
            ...
            [endpoint_manager_id for endpoint_manager_id, _ in self.queue.appended],
        )

    def test_queue_failure_keeps_prior_batch_and_does_not_compensate(self) -> None:
        self.queue = RecordingDeliverSeedQueue(fail_on="endpoint-2")
        self.pacer.deliver_seed_queue = self.queue
        self.task_score.acquire_dispatch_work_tasks.return_value = ("task-1",)
        self.dispatch_runtime.consume_candidate_workers.return_value = (
            self.candidate("worker-1", endpoint_manager_id="endpoint-1"),
            self.candidate("worker-2", endpoint_manager_id="endpoint-2"),
        ...
        self.assertEqual(["endpoint-1"], [entry[0] for entry in self.queue.appended])
        self.assertEqual(
            ["endpoint-1", "endpoint-2"],
            [entry[0] for entry in self.queue.attempts],
        )
        self.assertEqual(
