...
class RedisZsetWorkerScoreCore(WorkerScoreCore):
...
"""
    ...
    _ACQUIRE_DUE_HOT_LEASE_SCRIPT: ClassVar[str] = """
local key = KEYS[1]
local worker_id = ARGV[1]
local now_min_score = tonumber(ARGV[2])
local target_min_score = tonumber(ARGV[3])
local slot_factor = tonumber(ARGV[4])
local dirty_factor = tonumber(ARGV[5])
...
if stored_score >= now_min_score then
  return {"stale", stored_score}
end
if target_min_score <= now_min_score then
  return {"invalid", stored_score}
end

local slot_remainder = stored_score % slot_factor
local lane_rank = math.floor(slot_remainder / dirty_factor)
local next_score = target_min_score + lane_rank * dirty_factor
redis.call("ZADD", key, next_score, worker_id)
return {"transitioned", next_score}
"""
...
"""
    ...
    def acquire_hot_acquire_candidates(
        ...
        *,
        ...
        return [
            worker_id
            for worker_id, _ in self._range_worker_candidates(
    ...
    def acquire_due_hot_score_lease(
        ...
        *,
        ...
        return self._script_result(
            self.redis.eval(
                self._ACQUIRE_DUE_HOT_LEASE_SCRIPT,
                1,
                self._score_key(home_bucket_id),
                worker_id,
                self._abs_score(
