...
public class BasicMessageHandlerRegistrar {
    ...
    private List<MassMessage<?>> handlePing(MassMessage<?> msg) {
        // 构建一个 PONG 消息
        MassMessage<MessageResult> pong = new MassMessage<>();
        ...
        pong.setPayload(new MessageResult(200, "pong"));
    ...
    private List<MassMessage<?>> handleTask(MassMessage<?> msg) {
        ...
        // 这里只返回一个 ACK
        MassMessage<MessageResult> ack = new MassMessage<>();
        ...
        ack.setPayload(new MessageResult(200, "task received"));
