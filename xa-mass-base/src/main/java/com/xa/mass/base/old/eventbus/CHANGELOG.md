...
### ✨ 新特性与架构升级
- **新增** `StreamEventBusFacade`：通用事件总线门面，支持注入任意 `MessageStream` 实现（内存、Redis等）
- **新增** `MessageStream` 抽象接口，统一消息流操作，构造签名统一
- **新增** `InMemoryMessageStream`：高性能内存流实现，极大提升测试与开发效率
- **新增** `LettuceRedisStream`：Redis流实现，支持分布式与高可用
- **新增** `MessageStreamFactory`：工厂方法，一行切换内存/Redis实现
- **新增** `StreamEventBusExample`：最佳实践示例，支持多事件类型、批量、注销等
- **新增** `StreamEventBusFacadeTest`：单元测试用例，覆盖注册、分发、批量、注销等主流程
- **优化** 日志与异常处理，全部采用slf4j，注释更规范
- **完善** 文档与注释，便于团队理解和维护

### 🧪 测试增强
- **单元测试**：基于内存流的事件总线测试用例，支持多场景回归
- **example**：可配置切换内存/Redis，便于本地开发和生产部署

### 🔄 兼容性
- ✅ **完全向后兼容** - 现有API和用法保持不变
- ✅ **行为一致性** - 事件分发、注册、异常处理等逻辑一致
