...
ctx.setParameter("includeFailedDetail", true);
FilterResult<Device> result = filterProcessor.filter(devices, filterDef, ctx);
System.out.println("通过: " + result.getPassed().size());
System.out.println("拒绝率: " + result.getRejectRate());
if (result.getFailed() != null) {
    for (FilterReport.FilterFail<Device> fail : result.getFailed()) {
        System.out.println("设备: " + fail.getObject().getDeviceId() + ", 原因: " + fail.getFailedConditions());
    }
}
```
