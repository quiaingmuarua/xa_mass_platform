...
| `RedisStreamEventBusExample.java` | Redis分布式事件总线示例 | 跨进程/跨服务通信 |
| `DeviceEventListenerService.java` | 长期运行的事件监听服务 | 后台服务、微服务 |
...
```bash
cd xa-mass-base
java -cp "target/classes" com.xa.mass.base.eventbus.example.GuavaEventBusExample
```
...
java -cp "target/classes:$REDIS_LIBS" \
  com.xa.mass.base.eventbus.example.RedisStreamEventBusExample
```
...
public void onDeviceOffline(DeviceOfflineEvent event) {
    ...
    // ❌ 抛出未处理异常
    throw new RuntimeException("Something wrong");  // 影响其他监听器
}
```
...
public void onTaskCreated(TaskCreatedEvent event) {
    try {
        ...
        // 业务异常 - 记录日志，可能需要重试
        log.warn("处理任务失败，稍后重试: {}", event.getTask().getTid(), e);
        scheduleRetry(event);
    } catch (Exception e) {
        // 系统异常 - 记录错误，跳过处理
        log.error("处理任务时发生系统错误: {}", event.getTask().getTid(), e);
...
RedisStreamEventBusFacade eventBus = new RedisStreamEventBusFacade(
    "mass_events_v2",           // 使用版本化的Stream名
    "mass_consumer_group",      // 消费者组名
...
if (eventBus instanceof RedisStreamEventBusFacade) {
    ...
    log.info("EventBus状态: {}", redisEventBus.getStreamInfo());
...
public void performanceTest() {
    ...
    assertTrue("平均延迟应小于0.01ms", avgLatency < 0.01);
}
```

## 📞 获取帮助

- **问题排查**: 查看日志输出，检查Redis连接状态
- **性能问题**: 使用内置监控指标分析瓶颈
- **功能扩展**: 参考core/ARCHITECTURE.md了解扩展点
- **最佳实践**: 参考本文档的推荐做法部分 