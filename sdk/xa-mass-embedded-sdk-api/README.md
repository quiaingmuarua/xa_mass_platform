...
Status: current embedded SDK API contract owner README.

`xa-mass-embedded-sdk-api` owns embedded SDK-facing contract types that should
be usable by embedding callers without pulling in runtime composition internals.
It is not the public HTTP wire-contract module; that narrower owner is
...
- shared embedded SDK contract surface used by `xa-mass-embedded-sdk`, `xa-mass-server`, and
  tests
...
- `xa-mass-embedded-sdk-api` is the contract artifact
- `xa-mass-embedded-sdk` is the embedding/runtime-composition artifact
