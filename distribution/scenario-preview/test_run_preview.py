...
class PreviewLifecycleTest(unittest.TestCase):
    ...
    def test_build_uses_the_distribution_executable_and_independent_host(self):
        ...
        self.assertIn(":distribution:scenario-preview:stageServer", command)
        self.assertIn(":distribution:scenario-preview:stageFrontend", command)
        self.assertIn(":distribution:scenario-preview:stageHost", command)
        ...
        for packaged in (False, True):
            with self.subTest(packaged=packaged), tempfile.TemporaryDirectory() as directory:
                ...
                with patch("run_preview.redis.Redis.from_url", return_value=client), \
                    ...
                    self.assertIn("--spring.config.additional-location=" + (root / ("config" if packaged else "build/config")).as_uri() + "/", server.args[1])
