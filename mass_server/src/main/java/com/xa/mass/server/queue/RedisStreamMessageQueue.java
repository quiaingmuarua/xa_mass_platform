    ...
    private void initializeGroup() {
        try {
            ...
            String result = streamCommands.xgroupCreate(
                    ...
                    groupName,
                    XGroupCreateArgs.Builder.mkstream(true) // 如果流不存在则创建
            );
...
//        }
        ...
        try {
            ...
            if (messages != null && !messages.isEmpty()) {
                ...
                try {
                    ...
                    // 不 ACK，消息会保留在流中，并可能在超时后被重新传递给其他消费者或当前消费者
                    // 考虑死信队列 (DLQ) 策略
                    return null;
                }
            }
        } catch (RedisCommandTimeoutException e) {
            // 这是预期的行为，当在指定的 block 时间内没有消息时
    ...
    public boolean isEmpty() {
        try {
            // XLEN 返回流中的消息总数。对于消费者来说，"空"意味着没有待处理的新消息。
            // 一个更准确的检查是尝试非阻塞地 poll 一次。
            // 或者检查 XINFO GROUPS <stream_key> <group_name> 中的 pending 和 lag
            return streamCommands.xlen(streamKey) == 0; // 简化版：如果流中没有消息，则为空
        } catch (Exception e) {
    ...
    public void destroy() {
        ...
        this.active = false;
        // Lettuce 连接通常由 RedisClient 或 Spring 管理其生命周期，
        // 这里不需要显式关闭 connection，除非这个队列独占它。
    }
}