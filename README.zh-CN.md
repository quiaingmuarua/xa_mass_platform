...
它解决的不是“保存一条任务记录然后查状态”的 CRUD 问题，而是一个更偏
运行时内核的问题：

```text
一批结构化 work items
  -> 按 eventCode / capability / routing / reachability 匹配 worker
  -> 通过 transport 分发
  -> worker 执行并回写结果
  -> runtime 收敛每个 item 和整个 task 的状态
```
...
3. [architecture/add-worker-and-event.md](./architecture/add-worker-and-event.md)
   - 如何添加 event 和 worker capability
4. [xa-mass-sdk/README.md](./xa-mass-sdk/README.md)
   - SDK 嵌入式使用入口
5. [doc/EXTERNAL_WORKER_QUICKSTART.md](./doc/EXTERNAL_WORKER_QUICKSTART.md)
...
当前项目已经不是一个简单的 `engine + demo` 结构，而是在逐步形成完整的
产品壳和运行时内核边界：
...
- `platform_infra`
  ...
  - `TaskWorkRuntime` 和 `TaskResultRuntime` 是当前热路径核心 truth
- `transport`
  - worker delivery、presence、result ingress 的数据平面
  - polling / websocket / socket 是 peer adapters
- `xa-mass-sdk`
  - 推荐的 JVM 集成入口
...
当前主要有两类 task contract：

- `BATCH`
  - 更适合批量任务
  - intake sealed 后，所有 item final 可以推动 task 自动 terminal
- `SESSION`
  - 更适合会话式或持续接收消息的任务
  - 当前 work 集合清空不代表 session 结束

### Event

`eventCode` 是 capability identity。

worker 能不能处理某类 item，核心看它是否声明了对应 event binding。
...
这些是描述性 metadata 和 policy input，不是 runtime truth。它们不能直接决定：
...
当前 worker capability 主线已经从旧的 context 思路收敛到 worker/group/index：
...
真正能不能 dispatch，还要经过 engine 的 matching、rules、capacity、lock/resource policy。

### Result

public result read 已经收敛到 runtime-owned result truth：
...
| 模块 | 作用 |
| --- | --- |
| `xa-mass-base` | shared base models、command/event runtime vocabulary、低层 channel primitives |
...
- engine 是 runtime kernel，不是 CRUD backend。
- worker registration 不是 worker online。
- transport online 不是 worker capability。
- `eventCode` 不是 task type，而是 capability identity。
- trace 是 evidence，不是 runtime truth。
- projection 是 compatibility/debug/audit residue，不是 public result truth。
- 统一 event language 不等于统一 runtime，也不等于一个 generic event owner。

## 下一步

如果你想快速跑起来：

- 看 [architecture/quick-start.md](./architecture/quick-start.md)
- 再看 [xa-mass-sdk/README.md](./xa-mass-sdk/README.md)
...
- 然后按模块看 owner README，例如 [xa-mass-engine/README.md](./xa-mass-engine/README.md)

## 总结

XA Mass Platform 可以理解成一个面向“批量任务 + 会话任务”的分布式运行时调度内核。
