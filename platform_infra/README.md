...
- `xa-mass-engine` still owns `InMemoryRuleStorage` and engine-side storage usage, but the shared task/worker in-memory implementations and storage contracts no longer live under the engine package root
