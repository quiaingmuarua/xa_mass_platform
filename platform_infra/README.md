...
- `xa-mass-engine` still owns `InMemoryRuleStorage` and engine-side storage usage, but engine no longer carries Redis storage placeholder classes or the shared task/worker in-memory implementations under its package root
