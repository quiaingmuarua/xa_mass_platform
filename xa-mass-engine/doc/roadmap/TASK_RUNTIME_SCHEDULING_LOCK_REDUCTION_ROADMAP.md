...
| `TRS-D2` | Started | `TaskFlowLoadModelRunner` can run the same task append -> runtime enqueue -> runtime-ready pump -> match/reserve -> dispatch -> result -> release/refill -> terminal flow against memory or Redis runtime. The report now includes backend, stable-final result count, duplicate dispatch items, processing counter drift, result counter drift, and claimed messages/sec. |
