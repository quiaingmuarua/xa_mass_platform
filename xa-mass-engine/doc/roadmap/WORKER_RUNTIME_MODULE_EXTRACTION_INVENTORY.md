...
| Worker row lookup | `worker`, `workers`, `getWorker` | SDK worker shell reads, SDK diagnostics, tests | `WorkerResourceOwner` through `WorkerResourceRuntime` | control-plane storage read | Runtime API exposes `worker` / `workers`; SDK, transport, and perf matching loops no longer read `WorkerStorage` or model-shaped `WorkerManager` lookup helpers for worker shell/candidate lookup; the old `WorkerManager.getAllWorkers()` model-list helper and separate `WorkerLookupStore` seam have been deleted |
