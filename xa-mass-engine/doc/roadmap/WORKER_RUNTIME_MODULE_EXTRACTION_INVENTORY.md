...
| `WorkerStateProjectionRuntime` | `WorkerStateProjectionOwner` | Runtime-api contract for bounded worker state report projection |
...
(`WorkerStateReport`, `WorkerStateProjection`, `WorkerStateProjectionRuntime`, `WorkerStateProjectionResult`,
...
| --- | --- | --- | --- | --- | --- |
| Worker row mutation | `addWorker`, `updateWorker`, `deleteWorker` | SDK registration, SDK worker shell update, tests, server bootstrap through SDK | `WorkerResourceOwner` through `WorkerResourceRuntime` | control-plane storage plus derived runtime projection | `addWorker` / `updateWorker` now accept runtime-neutral `WorkerResourceRecord`; the old `Worker` model-shaped `WorkerManager` overloads have been deleted |
| Worker row lookup | `worker`, `workers` | SDK worker shell reads, SDK diagnostics, tests | `WorkerResourceOwner` through `WorkerResourceRuntime` | control-plane storage read | Runtime API exposes `worker` / `workers`; SDK, transport, and perf matching loops no longer read `WorkerStorage` or model-shaped `WorkerManager` lookup helpers for worker shell/candidate lookup; the old `WorkerManager.getWorker()` / `getAllWorkers()` model-list helpers and separate `WorkerLookupStore` seam have been deleted |
...
| Exclusive lease | `tryAcquireWorkerExclusiveLease`, `releaseWorkerExclusiveLease`, `hasWorkerExclusiveLease` | match strategy, resource releaser, diagnostics, tests | `WorkerAdmissionOwner` through `WorkerAdmissionRuntime` | runtime state | Rename away from lock vocabulary only when owner move requires it; the unused `getExclusiveLeaseWorkerIds()` forwarding helper has been deleted from the runtime contract and engine assembly surface |
