...
| `foreground` | Current public execution vocabulary and internal mechanism input. | Controls worker dispatch resource usage: `true` keeps the long-lived exclusive worker lock path; `false` relies on process-local capacity reservation for stateless/background sharing. | Keep as explicit execution-mode vocabulary. Do not reframe it as a general worker policy product without successor proof. |
