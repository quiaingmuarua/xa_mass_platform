...
| --- | --- | --- | --- | --- |
| A task that is not schedulable must not dispatch even if ready work exists | task lifecycle + assignment signal admission | `TaskSchedulingContentionTest`, `TaskPolicySchedulingOutcomeTest` | task status, runtime ready/inflight, no active lease | Covered |
