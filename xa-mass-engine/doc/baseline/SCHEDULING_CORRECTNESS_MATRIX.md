...
correctness. Projection residue and audit belong to `EngineProjectionResidueSuite`
and `EngineProjectionAuditSuite`.
...
| `EngineProjectionAuditSuite` | `xa-mass-engine` secondary | Projection audit boundary only |
...
| Candidate narrowing must use WorkerGroup event binding truth and must not fall back to all-worker scans | worker registry snapshot + `WorkerCandidateIndex` + candidate enumerator | `WorkerCandidateIndexTest`, `WorkerRegistrySnapshotTest`, `WorkerSchedulingCandidateEnumeratorTest`, `EngineSchedulingCoreArchitectureGuardTest` | indexed candidate source, source guards, assignment trace fields | Covered |
| `targetWorkerId` is a direct lookup shortcut, not a policy bypass | candidate index + Stage-2 admission | `TaskSchedulingGateAndTargetingTest`, `WorkerCandidateIndexTest`, `WorkerSchedulingCandidateEnumeratorTest` | no backup candidate, capability gate, lock/resource state | Covered |
