...
| `resultFinalityPolicy` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.resultFinalityPolicy` | `TaskRuntimeServingLane` result path via task-runtime `ResultApplyCommand` / `ResultFinalityPolicySnapshot` | hard-proven mechanism + resolved consumer guard | Result/lifecycle tests cover batch-style lease expiry failure and session-style expiry behavior through task-runtime result/finality ports; architecture guards block the deleted `TaskResultService` helper path from returning | Public `TaskContract` can still be mistaken as finality owner in docs/tests | Keep lifecycle proof and docs wording around resolved result finality |
| `backpressureClass` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.backpressurePolicy` | `TaskRuntimeServingLane` append path via task-runtime `AppendBatchCommand` / `AppendAdmissionPolicy` | hard-proven mechanism + resolved consumer guard | `TaskKernelLifecycleTest#interactiveTaskAppendRespectsWorkloadAwareReadyBackpressureCap` covers ready/intake backpressure; production append paths pass resolved policy into the task-runtime append command path; architecture guards block old enqueue option resolver return | Raw profile reads can still bypass resolved policy if reintroduced outside the serving lane | Keep resolved consumer guard and lifecycle backpressure proof |
