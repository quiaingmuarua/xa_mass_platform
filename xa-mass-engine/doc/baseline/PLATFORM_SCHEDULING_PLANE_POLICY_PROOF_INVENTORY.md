...
| `routeAttributes` | `Task.sharedConfig.routeAttributes` | `TaskDispatchIntent`, `ResolvedWorkerSchedulingPolicy` | candidate bucket policy / worker candidate buckets where configured | resolved-only/unproven in engine-local policy proof | Worker-runtime index and server trace tests cover candidate-bucket mechanics, but current engine-local harness does not produce a distinct route-attribute scheduling outcome | Easy to overclaim because resolver tests assert map copying | Keep `resolved-only/unproven`; do not implement or test route-attribute scheduling without a separate owner decision |
| `candidateBucketKeys` | `WorkerCandidateBucketPolicy` derived from approved task route attributes | `ResolvedWorkerSchedulingPolicy` | `WorkerTaskSelectorFactory#fromPolicy` -> bounded worker candidate source | support-regression; hard outcome unproven | Worker-runtime index tests cover candidate-bucket mechanics, but current engine-local hard proof is tied to group/routing outcomes rather than bucket-key perturbation alone | Candidate-source guard can look proven by source scans only | Keep outside hard policy proof until candidate-bucket perturbation changes runtime assignment outcome without turning `routeAttributes` into an implicit policy product |
...
| `workloadClass` | `Task.executionSpec.workloadClass` as preset input | `ResolvedTaskSchedulingPolicy` | `DefaultWorkerBudgetPolicy`, `TaskAssignWorker`, claim/retry/backpressure policy adapters | hard-proven for current outcomes | `TaskSchedulingContentionTest#largeBulkTaskIsCappedAndLeavesWorkersForInteractiveTask` observes bulk cap, remaining ready/inflight counts, interactive lease; `TaskAssignWorkerTest#retryDelayUsesResolvedWorkloadPolicy` observes retry delay trace; architecture guards require claim/retry/backpressure consumers to use resolved policy | Workload class can still be mistaken as direct behavior truth if new owners bypass `ResolvedTaskSchedulingPolicy` | Keep hard proof for budget/retry outcomes and guard production consumers from raw profile reads |
...
| `batchPolicy` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.claimPolicy` | `TaskRuntimeClaimOptionsResolver` via resolved claim policy | hard-proven mechanism + resolved consumer guard | Claim option tests cover per-worker capacity; `SimpleTaskDispatchBinder` production path passes resolved task policy; architecture guard blocks raw task claim consumption in production binder | Raw resolver overload is package-private support and must not become production mainline | Keep resolved consumer guard and claim option tests |
| `leaseProfile` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.claimPolicy` | `TaskRuntimeClaimOptionsResolver` via resolved claim policy | hard-proven mechanism + resolved consumer guard | Claim option tests cover lease duration; redispatch/expiry tests cover hard runtime lease behavior; production binder passes resolved task policy | Raw resolver overload is package-private support and must not become production mainline | Keep resolved consumer guard and lease-expiry lifecycle tests |
| `resultFinalityPolicy` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.resultFinalityPolicy` | `TaskRuntimeServingLane` result path via task-runtime `RuntimeResultFact` and runtime-owned result policy meta | hard-proven mechanism + resolved consumer guard | Result/lifecycle tests cover batch-style lease expiry failure and session-style expiry behavior through task-runtime result/finality ports; architecture guards block the deleted `TaskResultService` helper path from returning and old result command DTOs from re-entering | Public `TaskContract` can still be mistaken as finality owner in docs/tests | Keep lifecycle proof and docs wording around resolved result finality |
| `backpressureClass` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.backpressurePolicy` | `TaskRuntimeServingLane` append path via task-runtime `AppendItemInput` / `TaskRuntimeWorkPort#appendBacklog` | hard-proven mechanism + resolved consumer guard | `TaskKernelLifecycleTest#interactiveTaskAppendRespectsWorkloadAwareReadyBackpressureCap` covers ready/intake backpressure; production append paths pass caller-owned accepted backlog items into the task-runtime append path; architecture guards block old enqueue option resolver return and old append command DTOs from re-entering | Raw profile reads can still bypass resolved policy if reintroduced outside the serving lane | Keep resolved consumer guard and lifecycle backpressure proof |
...
| foreground/background resource semantics | `Task.executionSpec.foreground` as preset input | `ResolvedTaskSchedulingPolicy.workerResourceMode` | `WorkerDispatchResourcePolicy` via resolved resource mode | hard-proven mechanism + resolved consumer guard; not public policy product | `TaskSchedulingContentionTest#backgroundTasksShareStatelessWorkerUpToDeclaredCapacity` observes shared active lease count, no exclusive lock, quota rejection, and task status; architecture guard blocks resource policy from reading foreground directly | Treating foreground/background as public policy product before vocabulary decision would be premature | Keep in scheduling correctness matrix and resolved resource-mode guard |
