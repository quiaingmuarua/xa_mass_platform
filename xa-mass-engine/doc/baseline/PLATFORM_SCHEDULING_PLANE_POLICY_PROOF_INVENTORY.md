...
| `backpressureClass` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.backpressurePolicy` | `TaskRuntimeServingLane` append path via task-runtime `BacklogFrameV1` / `TaskRuntimeWorkPort#appendBacklog` | hard-proven mechanism + resolved consumer guard | `TaskKernelLifecycleTest#interactiveTaskAppendRespectsWorkloadAwareReadyBackpressureCap` covers ready/intake backpressure; production append paths pass accepted backlog frames into the task-runtime append path; architecture guards block old enqueue option resolver return and old append command DTOs from re-entering | Raw profile reads can still bypass resolved policy if reintroduced outside the serving lane | Keep resolved consumer guard and lifecycle backpressure proof |
