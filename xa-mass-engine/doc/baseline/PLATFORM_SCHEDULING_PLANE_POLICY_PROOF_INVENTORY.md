...
| `resultFinalityPolicy` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.resultFinalityPolicy` | `TaskResultService` | hard-proven mechanism + resolved consumer guard | Result/lifecycle tests cover batch-style lease expiry failure and session-style expiry behavior; architecture guard blocks `TaskResultService` from using temporary preset semantics | Public `TaskContract` can still be mistaken as finality owner in docs/tests | Keep lifecycle proof and docs wording around resolved result finality |
| `backpressureClass` | derived by task policy preset resolution | `ResolvedTaskSchedulingPolicy.backpressurePolicy` | `TaskRuntimeEnqueueOptionsResolver` via resolved backpressure policy | hard-proven mechanism + resolved consumer guard | `TaskKernelLifecycleTest#interactiveTaskAppendRespectsWorkloadAwareReadyBackpressureCap` covers ready/intake backpressure; production append paths pass resolved policy; architecture guard blocks raw task enqueue consumption | Raw resolver overload is package-private support and must not become production mainline | Keep resolved consumer guard and lifecycle backpressure proof |
