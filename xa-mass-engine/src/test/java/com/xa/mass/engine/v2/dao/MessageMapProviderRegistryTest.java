...
public class MessageMapProviderRegistryTest {
    ...
    void testCreateInMemoryMap() {
        ...
        MessageMap<String, TaskEntity> map = MessageMapProviderRegistry.createMap(
            MessageProviderType.IN_MEMORY, "test-map", TaskEntity.class);
    ...
    void testCreateMapWithDefaultName() {
        // 测试使用默认名称创建映射
        MessageMap< String,Integer> map = MessageMapProviderRegistry.createMap(MessageProviderType.IN_MEMORY,"KEY",Integer.class);
...
//        assertEquals("default", map.getName());
    ...
    void testCreateMapWithProjectName() {
        // 测试使用项目名称创建映射
        MessageMap<String, TaskEntity> map = MessageMapProviderRegistry.createMap(
            MessageProviderType.IN_MEMORY, "task:DEMO", TaskEntity.class);
    ...
    void testCreateRedisMapWithTypeInfo() {
        ...
        assertThrows(Exception.class, () -> {
            MessageMapProviderRegistry.createMap(
                MessageProviderType.REDIS, "test-map", TaskEntity.class);
        });
    }
    ...
    void testCacheReuse() {
        ...
        MessageMap<String, TaskEntity> map1 = MessageMapProviderRegistry.createMap(
            MessageProviderType.IN_MEMORY, "cache-test", TaskEntity.class);
    ...
    void testDifferentTypesCreateDifferentInstances() {
        // 测试不同类型创建不同实例
        MessageMap<String, TaskEntity> map1 = MessageMapProviderRegistry.createMap(
            MessageProviderType.IN_MEMORY, "test", TaskEntity.class);
        MessageMap<String, Integer> map2 = MessageMapProviderRegistry.createMap(
            MessageProviderType.IN_MEMORY, "test", Integer.class);
    ...
    void testHasProvider() {
        // 测试提供者检查
        assertTrue(MessageMapProviderRegistry.hasProvider(MessageProviderType.IN_MEMORY));
        assertTrue(MessageMapProviderRegistry.hasProvider(MessageProviderType.REDIS));
        assertFalse(MessageMapProviderRegistry.hasProvider(MessageProviderType.KAFKA));
    }
} 