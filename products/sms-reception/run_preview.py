"""Run the finite SMS preview from the checkout or an extracted preview ZIP."""
...
class Preview:
    def __init__(self, counts=(20, 20, 20), port=18390, redis_url=None, root=PRODUCT, output=None):
        ...
        self.scope = "test_sms_" + uuid.uuid4().hex
    ...
    def close(self):
        ...
        if self.redis is not None:
            if not re.fullmatch(r"test_sms_[0-9a-f]{32}", self.scope):
...
def main():
    ...
    parser.add_argument("--port", type=int, default=18390, help="Server base port; Adapter +3 and Host +4")
