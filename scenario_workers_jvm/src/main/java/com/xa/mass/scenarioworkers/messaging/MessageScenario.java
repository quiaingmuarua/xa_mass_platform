...
public final class MessageScenario implements AutoCloseable {
    ...
    public Sender addSender(String group, String replica, String country, String phone,
            Supplier<String> workerId, Supplier<String> runtimeState) {
        synchronized (gate) {
            var sender = new Sender(group, replica, country, phone, workerId, runtimeState);
            if (senders.putIfAbsent(phone, sender) != null) throw new IllegalArgumentException("Duplicate sender");
            return sender;
        }
    }
    ...
    public Map<String, Object> send(Sender sender, Map<String, Object> request, WorkerOutcomeReporter reporter) {
        ...
        if (!sender.country.equals(text(request, "country", 2))) throw new IllegalArgumentException("Wrong sender country");
        synchronized (gate) {
            ...
            if (existingCountry != null && !existingCountry.equals(sender.country)) throw new IllegalArgumentException("Campaign country conflict");
            campaigns.put(campaign, sender.country);
            Entry entry = new Entry(Map.copyOf(request), sender, reporter);
            var snapshot = new LinkedHashMap<String, Object>(request);
            snapshot.put("status", "SENT"); snapshot.put("workerId", worker); snapshot.put("phone", sender.phone);
    ...
    public Map<String, Object> inventory(int offset, int limit) {
        ...
        synchronized (gate) {
            return Map.of("total", senders.size(), "items", senders.values().stream().skip(offset).limit(limit).map(sender -> {
                ...
                item.put("workerGroupId", sender.group); item.put("replicaKey", sender.replica); item.put("country", sender.country);
                item.put("phone", sender.phone); item.put("workerId", sender.workerId.get()); item.put("runtimeState", sender.runtimeState.get());
                return item;
            }).toList());
        }
    }
    ...
    public void stop(String phone) {
        synchronized (gate) {
            Sender sender = senders.get(phone);
    ...
    public void start(String phone) {
        synchronized (gate) {
            if (closed) throw new IllegalStateException("Message channel closed");
            Sender sender = senders.get(phone);
            if (sender != null) sender.accepting = true;
        }
    }
    ...
    public static final class Sender {
        final String group, replica, country, phone;
        final Supplier<String> workerId, runtimeState;
        final List<Entry> messages = new ArrayList<>();
        boolean accepting = true;
        Sender(String group, String replica, String country, String phone, Supplier<String> workerId, Supplier<String> runtimeState) {
            this.group = group; this.replica = replica; this.country = country; this.phone = phone;
