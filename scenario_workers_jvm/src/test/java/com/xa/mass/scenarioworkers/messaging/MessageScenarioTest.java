package com.xa.mass.scenarioworkers.messaging;
...
class MessageScenarioTest {
    ...
    @Test void handlerDeduplicatesAcrossWorkersAndRetainsFirstReporter() {
        try (var host = new MessageScenario()) {
            ...
            assertThat(host.act("m", "deliver", Map.of())).containsEntry("sendAccepted", true);
    ...
    @Test void receiptReleaseCanReorderAndDuplicateOnlyCommittedFacts() {
        try (var host = new MessageScenario()) {
            ...
            host.send(sender(host, "one"), send("m"), (tag, time, payload) -> { reports.add(Jsons.parseObject(payload)); return true; });
            host.hold(true);
            String delivered = (String) host.act("m", "deliver", Map.of()).get("receiptId");
            String old = (String) host.act("m", "reply", Map.of("requestId", "r1", "text", "old")).get("receiptId");
            String latest = (String) host.act("m", "reply", Map.of("requestId", "r2", "text", "latest")).get("receiptId");
            assertThat(host.act("m", "reply", Map.of("requestId", "r1", "text", "old"))).containsEntry("unchanged", true);
    ...
    @Test void publishRunsOutsideGateAndStopDoesNotTransferOldReporter() throws Exception {
        try (var host = new MessageScenario(); var executor = Executors.newSingleThreadExecutor()) {
            ...
            var action = executor.submit(() -> host.act("old", "deliver", Map.of()));
            assertThat(entered.await(2, TimeUnit.SECONDS)).isTrue();
            host.stop("phone-one"); host.start("phone-one");
            assertThat(host.act("old", "read", Map.of())).containsEntry("persisted", true).containsEntry("sendAccepted", false);
            ...
            assertThat(host.act("old", "reply", Map.of("requestId", "r", "text", "reply"))).containsEntry("sendAccepted", false);
            host.send(sender, send("new"), (tag, time, payload) -> true);
            assertThat(host.act("new", "deliver", Map.of())).containsEntry("sendAccepted", true);
        }
    }
    @Test void failedSendRetainsLocalFactWithoutRetry() {
        try (var host = new MessageScenario()) {
            host.send(sender(host, "one"), send("m"), (tag, time, payload) -> { throw new IllegalStateException("offline"); });
            assertThat(host.act("m", "deliver", Map.of())).containsEntry("persisted", true).containsEntry("sendAccepted", false);
            assertThat(host.act("m", "deliver", Map.of())).containsEntry("unchanged", true);
    ...
    @Test void capacityRejectionCannotPartiallyCommitARecipientAction() {
        try (var host = new MessageScenario()) {
            host.send(sender(host, "one"), send("m"), WorkerOutcomeReporter.UNAVAILABLE);
            host.hold(true); host.act("m", "deliver", Map.of());
            for (int i = 1; i < MessageScenario.MAX_HELD; i++)
                host.act("m", "reply", Map.of("requestId", "r" + i, "text", "reply" + i));
            var previous = host.page(0, 1);
            assertThatThrownBy(() -> host.act("m", "reply", Map.of("requestId", "overflow", "text", "not committed")))
