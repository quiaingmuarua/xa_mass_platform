...
- 不要默认 README 或历史设计文档描述的是当前实现
- 如果代码、运行表现、文档三者冲突，优先相信代码和已验证运行行为
- 发现事实变化后，先修当前文档，再考虑保留历史说明

## 2. 当前主线判断

- 真实 Spring Boot 入口是 `xa-mass-mock`
- `xa-mass-starter` 不是当前可直接运行的 Boot 入口
- API-first 任务流是当前主线真相，UI 页面只是辅助观察面
- `engine/v2` 不是当前生产主线
...
- 当前真实可运行入口
- 串起 `api + starter + gateway + engine`
- 用于全链路验证和 mock 数据加载

### `xa-mass-starter`

- 生命周期 / 组装层
- 负责构建和启动 `MassApplication`、`MassEngine`、`MassGateway`
- 不是当前 `spring-boot:run` 入口

### `xa-mass-api`

- REST controller、状态页、模板层
- 由 `xa-mass-mock` 的 Spring Boot 扫描加载
- 不是独立验证过的应用入口

### `xa-mass-engine`

- 主线业务逻辑所在
- 当前应优先关注 `TaskManager`、`DeviceManager`、`RuleManager`
- `v2` 目录与其测试属于历史资产，不应默认纳入主线判断
...
- 入口应从 `xa-mass-mock` 开始
- 不要从 `xa-mass-starter` 倒推“它就是运行入口”

### 任务生命周期

- 生命周期的业务真相应以 `TaskManager` + `TaskStatus` 为准
- API 控制器当前已基本对齐 `TaskManager`
- 文档中若出现和 `TaskStatus.canTransitionTo()` 冲突的规则，应视为过时

### 事件总线

- 新旧事件总线并存
- 不要默认 `StreamEventBusFacade` 已完全替代 Guava 路径
- 讨论事件总线问题时，先看调用点，再看架构文档

### 测试面

- 主线回归应优先看 engine/api/mock 当前测试
- `xa-mass-engine/src/test/java/com/xa/mass/engine/v2/**` 属于历史测试债
...
- API 端到端覆盖仍偏薄，尤其是失败路径、终止路径、重复回调路径
...
- `xa-mass-gateway` 下的任务发布与结果处理链路

### 事件总线

- `xa-mass-starter/src/main/java/com/xa/mass/starter/MassEngine.java`
- `xa-mass-base` 下 `old.eventbus` 与 `channel.eventbus` 的实际调用点
...
- 先补测试再修行为
- 修完行为后同步更新当前文档
- 把历史说明留在 `doc/archive/`，不要再回流到主入口文档
