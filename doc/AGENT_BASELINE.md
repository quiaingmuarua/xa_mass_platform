...
- Runtime transport delivery has explicit backlog admission control and shutdown semantics. `InMemoryTransportDeliveryStore` enforces a total queued-item cap in addition to per-worker caps, exposes `TransportDeliveryStoreStats`, rejects new delivery after shutdown, and wakes waiting pollers so future Redis/JDBC stores can preserve the same HA diagnostics and lifecycle behavior.
