...
- Current SDK control-plane event dispatch through `InMemoryMassEventRuntime` is synchronous and has no hard handler timeout or handler isolation. Treat this as a known HA gap: direct runtime event handlers must stay short and cooperative until event dispatch is moved behind a bounded runtime executor with explicit timeout/cancellation semantics.
