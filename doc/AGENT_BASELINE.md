...
- Runtime transport delivery has explicit backlog admission control. `InMemoryTransportDeliveryStore` enforces a total queued-item cap in addition to per-worker caps and exposes `TransportDeliveryStoreStats` so future Redis/JDBC stores can preserve the same HA diagnostics.
