...
| `TRS-M3` | Implemented first slice | `EngineSchedulingCoreArchitectureGuardTest.taskWriteLockRemainsLifecycleAndProgressOnly` locks the current boundary: task write locks are allowed for lifecycle, intake, progress, and audit paths only; runtime claim and internal runtime result apply must stay task-lock free, and `withTaskWorkReadLock(...)` must remain a read/message guard rather than a write lock. `TaskResultConcurrencyConvergenceTest.successCallbacksForDifferentMessagesDoNotSerializeAtTaskLevel` proves same-task different-message results can enter runtime apply concurrently under the current read/message guard. |
...
| `TRS-D1` | Implemented source-scan slice | Trace proof now accepts only explicit group-first candidate sources, stale worker-dispatch-gate docs point to `WorkerRegistry` slot disabled sources, the transport worker spine roadmap no longer names the retired dispatch-availability owner as the source-gate mutation target, and the superseded worker-resource-occupancy roadmap has been removed. A current source scan found no non-archive production caller for retired dispatch-availability / worker-lock / WorkerLoadView truth; Redis keyspace tests remain current key contract tests after `TRS-C2`, not obsolete scheduling-truth residue. |
| `TRS-D2` | Implemented proof slice | `TaskFlowLoadModelRunner` can run the same task append -> runtime enqueue -> runtime-ready pump -> match/reserve -> dispatch -> result -> release/refill -> terminal flow against memory or Redis runtime. The report now includes backend, stable-final result count, duplicate/stale/expired runtime counters, processing counter drift, result counter drift, first-dispatch lag, claimed messages/sec, and runtime operation latency summaries for work/result runtime calls. It also has opt-in lease-expiry/refill proof (`mass.load.expireFirstAttemptEveryNth`), stale-result proof (`mass.load.staleResultEveryNth`), duplicate-result callback proof (`mass.load.duplicateResultEveryNth`), and duplicate-wakeup proof (`mass.load.duplicateWakeupsOnApprove`) without changing the default smoke path. `run-perf-smokes.sh` can opt into backend and fault/wakeup proof, and existing perf smokes have been converged to explicit WorkerGroup selector setup. Remaining D2 work is broader distributed worker disconnect/reconnect proof; that should not block the current task-runtime lock-reduction slice. |
...
| `applyTaskWorkResultWithContext(...)` | `TaskWorkRuntime` | runtime result-apply atomic boundary | Must not take task write/read locks; runtime implementation owns final apply idempotence and pre/post mutation context. |
...
3. Do not require task write lock merely to:
   ...
   - apply a runtime work result after the public convergence entry has passed
     the current conservative read/message guard
...
3. Do not delete Redis keyspace/runtime-shape tests merely because they mention
   current keys; after `TRS-C2`, they are key contract tests unless a future
   migration replaces the key shape with equivalent proof.
