...
This inventory records the old engine and worker-runtime worker-selection call
surface that the roadmap was written to remove. It is not current-state proof.
Current truth is in `xa-mass-engine/README.md`,
`xa-mass-worker-runtime/README.md`, and `xa-mass-worker-runtime/CONTRACTS.md`.

Current mainline outcome as of 2026-06-18:

- Engine consumes `WorkerSelectionRuntime`, `WorkerSelectionRequest`,
  `WorkerSelectionResult`, `SelectedWorkerHandle`, and
  `SelectedWorkerEvidence`.
- Engine production no longer contains `RuleBasedTaskWorkerMatchingStrategy`,
  `TaskWorkerMatchingStrategy`, `WorkerSchedulingCandidateEnumerator`,
  `WorkerSchedulingCandidate`, `WorkerSchedulingView`, `WorkerMatchContext`,
  or engine-side candidate ranker classes.
- Worker-runtime owns worker-fact predicate/ranking mechanics, reservation,
  exclusive lease acquisition, claim authorization, and selected-worker
  accounting behind the selection contract.
- SDK/server diagnostic read models remain out of this inventory's scope.

## Symbols

| Symbol | Current Owner | Current Caller | Classification | Target |
| --- | --- | --- | --- | --- |
| `EngineRuntimeKernelConfig#getWorkerCandidateRuntime()` | engine assembly | `EngineRuntimeKernel` | engine sees worker candidate-source port | Replace with approved selected-worker contract such as `selectAndReserve(...)`. |
| `EngineRuntimeKernelConfig#getWorkerSchedulingViewRuntime()` | engine assembly | `RuleBasedTaskWorkerMatchingStrategy` / `WorkerSchedulingCandidateEnumerator` | engine sees live worker evidence read port | Remove from engine assembly; worker-runtime composes evidence inside selected-worker mechanisms. |
...
| `WorkerSchedulingViewRuntime` | worker-runtime | `WorkerSchedulingCandidateEnumerator` | live reachability/load/group read evidence exposed to engine | Internal worker-fact evidence for selection mechanisms. Server/diagnostics use separate inspection view. |
...
| `WorkerMatchContext` | engine | `RuleBasedTaskWorkerMatchingStrategy`, rule evaluator, diagnostics | engine materializes worker metadata and live evidence | Remove worker metadata from engine rule context; route worker-side predicates through worker-runtime selection contracts. |
| `DefaultWorkerCandidateRanker` | engine | `RuleBasedTaskWorkerMatchingStrategy` | engine ranks by load, routing affinity, worker attributes | Move worker-fact ranking mechanics inside worker-runtime selected-worker mechanisms; policy choice stays in Scheduling Plane. |
| `RuleBasedTaskWorkerMatchingStrategy` | engine | `EngineRuntimeKernel` default strategy | engine performs candidate enumeration, prefilter, rule evaluation, ranking, reserve | Replace with engine construction of minimal worker selection request and selected-handle consumption. Do not move this class wholesale. |
| `TaskWorkerMatchingStrategy` | engine | `TaskWorkerAssignListener` | extension seam returns `WorkerSchedulingCandidate` | Replace or narrow so extensions choose task-side selection strategy or return minimal selected handles, not worker views or worker-runtime stage handles. |
...
| `sdk/xa-mass-embedded-sdk` | `xa-mass-engine` + `xa-mass-worker-runtime` | main | assembly of engine and worker runtime | Provide the approved selected-worker contract to engine config. |
...
- Worker attributes, WorkerGroup capability projections, worker load details,
  ...
  belong inside worker-runtime selected-worker mechanisms.
...
- Worker-runtime must not understand task lifecycle fields. It may carry opaque
  ...
  terminal policy.
- For this roadmap, engine passes worker-universe intent through WorkerGroup and
  approved routing intent only. It must not add a separate worker-universe
  partition or decide worker-runtime final source keys, cache keys, storage
  keys, or prewarm keys.
- Worker-runtime must not invent its own selection-visible shard, bucket, or
  ...
  mechanics derived from engine-provided worker-universe intent and
  canonical worker facts; they must not change the worker universe, ranking
  domain, or assignment correctness.
- `WorkerSelectionIntent`, `WorkerSelectionRequest`, selected handles, and
  selected-worker accounting commands belong in a worker-runtime selection API
  package. Only claim bridge values that must be consumed by task-runtime should
  move down to `mass-runtime-api`. Worker-runtime must keep no reverse
...
- The target is not moving the current fat engine strategy class into
  worker-runtime. Engine should build minimal worker selection requests and
  consume selected handles; worker-runtime should execute worker-fact mechanisms
  behind minimal handles and outcomes.
