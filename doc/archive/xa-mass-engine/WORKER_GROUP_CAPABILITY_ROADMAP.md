...
| sdk event candidate narrowing | `WorkerManager.findWorkerCandidates(task)` | historical `sdkEventCode` narrowing used worker-level supported-event storage lookup | WG-3 replaces worker supported-event lookup with `(project,eventCode) -> groupIds -> workerIds` |
