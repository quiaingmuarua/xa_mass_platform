...
Status: historical and archived on 2026-06-18.

This roadmap is superseded for worker-selection owner truth by
`doc/archive/xa-mass-engine/2026-06-18_WORKER_RUNTIME_SELECTION_MINIMAL_CONTRACT_ROADMAP.md`
and superseded for transport route-owner and dispatch-handoff details by the
transport baseline.

Do not execute this roadmap as a current implementation plan. It is retained as
historical residue context until any still-useful transport/reachability notes
are moved to the owning baseline or archive.
...
remaining worker-selection cleanup in this roadmap was replaced by
`doc/archive/xa-mass-engine/2026-06-18_WORKER_RUNTIME_SELECTION_MINIMAL_CONTRACT_ROADMAP.md`.
Any transport-specific cleanup must be re-read through the current transport
...
Do not use the older bullets in this file as current owner truth. Current
worker-selection owner truth is defined by
`doc/archive/xa-mass-engine/2026-06-18_WORKER_RUNTIME_SELECTION_MINIMAL_CONTRACT_ROADMAP.md`:

- Engine owns task-side scheduling policy, allocation budget, assignment
  orchestration, dispatch binding, retry/result convergence, and terminal
  policy.
- Worker-runtime owns worker-fact selection mechanics behind
  `WorkerSelectionRuntime`, including candidate acquisition, reachability,
  dispatch gate, load, capability predicate/ranking, reservation, selected
  claim authorization, and selected-worker accounting.
- Transport owns protocol sessions, delivery feasibility, and selected-worker
  delivery mechanics. It does not choose replacement workers.

## Historical Core Ownership (Superseded)

The following owner split was the pre-WSM residue frame and is retained only to
explain why this roadmap existed. It must not drive new implementation.
...
The previous idea of deriving `WorkerReachabilityView` from transport
route-owner lookup is also superseded; worker reachability/admission evidence
belongs to worker-runtime or an explicitly owned worker-system-event projection,
not to transport session lease evidence.
...
## Current Transport Guards And Historical Selection Rules

Rules 1-7 remain useful transport/reachability guardrails. Rules 8-12 describe
the retired engine-side match-context/candidate-ranking model and are
superseded by WSM selected-handle guards. They are retained only as historical
residue context.
...
8. Historical: rule evaluation consumed `WorkerMatchContext#getRuleContext()`,
   never
   full diagnostic context.
9. Historical: rule-readable context did not contain live reachability, dispatch gate,
   ...
   exception.
10. Historical: diagnostic context could explain a runtime-selection decision,
    but it did not
...
11. `ResolvedWorkerSchedulingPolicy` must not carry live runtime or transport
    evidence.
12. Historical: candidate ranking was read-only. It could order candidates; it
    did not
...
Historical production order (superseded by `WorkerSelectionRuntime`):
...
Current production worker-fact selection is hidden behind
`WorkerSelectionRuntime`; engine must not reintroduce candidate acquisition,
rule-context worker facts, ranking, or direct admission calls as separate
engine stages.

## Historical Slice Notes

The RWS-* slices and proof commands below are historical residue notes. They
reference retired tests and types such as `WorkerMatchContextTest` and
`RuleBasedTaskWorkerMatchingStrategyTest` because those were the old proof
surfaces. Do not run them as current completion gates; use WSM verification
instead.
