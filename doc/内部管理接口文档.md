...
> - 回归测试命令
>
> 这些内容统一见 [VERIFIED_RUNBOOK.md](./VERIFIED_RUNBOOK.md)
...
| `PUT` | `/status/api/tasks/{taskId}/status` | 状态路由辅助接口 | 通过 status 参数转发到 `approve/reject/pause/resume/cancel` |
...
- `READY -> RUNNING` 由分配链路驱动，不是独立的手工 API 动作
- `RUNNING -> PAUSED` 在 `TaskManager` 和 `TaskStatus` 中是允许的
- `READY -> BLOCKED` 和 `RUNNING -> BLOCKED` 在状态机里允许，但当前没有独立公开 API 直接触发这两个动作
