...
| `transport.worker-route-owner` | worker dispatch route ownership is keyed by opaque `routeKey`; route-key minting is outside transport runtime, one route has one current delivery owner, stale session tokens cannot revoke newer owners, delivery queues are route-key owned, and transport route selection does not decide worker capacity | transport runtime/API proof: `CanonicalWorkerRouteKeyCodecTest` for the SDK/starter default worker route-key codec, `RedisWorkerPresenceStoreTest`, `InMemoryWorkerPresenceStoreTest`, `WorkerDispatchRouteSelectorTest`, `NodeTargetedTaskDispatchSubmitterTest`, `TransportDeliveryServiceTest`, `RedisTransportDeliveryStoreTest`, `TransportRoutingTaskDispatchListenerTest` | SDK/server representative proof: `MassApplicationDistributedTransportTest`, `ExternalWorkerApiControllerTest`, `ExternalWorkerPollingApiIntegrationTest`, `ExternalWorkerRealtimeRegistrationIntegrationTest`, plus public Java SDK `WorkerClientTest`, `PollingWorkerSessionTest`, `WebSocketWorkerSessionTest` | no dedicated trace analyzer currently; route-owner proof is runtime/transport/API behavior until trace schema exposes route-owner events | add chaos only for real distributed route-owner churn, Redis presence failure, or adapter process takeover beyond current deterministic proof | `covered-wrb-current-slice` | do not use raw source scans, field-copy tests, or workerId route resolver helpers as route-owner proof; transport Redis must keep `workers`, `owner-shards`, `worker-routes:*`, `route-presence:*`, and worker runtime/admission keys out of transport truth |
