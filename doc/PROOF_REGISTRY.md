...
| `sched.worker-state-dimensions` | worker reachability, readiness/dispatch gate, and occupancy/capacity remain separate scheduling dimensions; non-reachable, draining/dispatch-disabled, locked, occupied, or capacity-full workers cannot bind work while eligible ready workers still can | `EngineSchedulingCoreSuite`: `TaskWorkerEligibilityTest`, `TaskSchedulingGateAndTargetingTest`, `TaskSchedulingContentionTest` | representative host proof reuses worker eligibility/routing and retry rows unless server read APIs expose new dimension fields | trace proof reuses assignment/retry analyzers unless trace schema gains explicit worker dimension fields | add chaos only when transport churn or Redis runtime degradation changes the dimension outcome | `covered-engine-current-slice` | current proof covers `READY`/`DRAINING`/`MAINTENANCE` over the canonical slot aggregate; target-only readiness values and Redis `worker:meta`/`worker:occupancy` split keys remain residue, and enum/getter/key-name tests are not proof |
