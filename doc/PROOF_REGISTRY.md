...
| `transport.delivery-command-fact-ownership` | assigned delivery commands are minimal item intents: command items carry selected worker plus `TaskDispatchContent` and `TaskDispatchExecutionContext` only; `adapterId`, `deliveryQueueKey`, `targetTransportNodeId`, route-owner evidence, connection/session facts, worker pull DTOs, generic task-dispatch `TransportPacket`, and generic correlation maps are owned by group, lane/store key, endpoint, final-hop projection, pull projection, or observation snapshots instead of per-command or per-envelope handoff payloads | transport runtime/API proof: `DeliveryCommandTest`, `PulledTaskDispatchTest`, `TaskPullResultTest`, `TransportDispatchEnvelopeTest`, `JsonTransportPacketCodecTest`, `DispatchOutcomeTest`, `TransportAssignedDeliverySubmitterTest`, `TransportDeliveryCommandListenerTest`, `TransportDeliveryCommandBatchCodecTest`, `InMemoryTransportDeliveryCommandHandoffTest`, `RedisTransportDeliveryCommandHandoffTest`, `RedisTransportDispatchEnvelopeCodecTest`, `RedisTransportDeliveryStoreTest`, `TransportDeliveryFailureEventCodecTest`, `RedisTransportDeliveryFailureChannelTest`, `TransportPacketFactoryTest`, `TransportConvergenceArchitectureGuardTest` | SDK/server representative proof: `MassApplicationDistributedTransportTest`, `ExternalWorkerApiControllerTest`, `ExternalWorkerPollingApiIntegrationTest#pollingWorkersSharingRouteAndQueueCannotCrossConsumeSelectedWorkerItems`, plus public Java SDK `WorkerClientTest`, `PollingWorkerSessionTest` | no dedicated trace analyzer currently; this is a transport process-boundary shape invariant until trace schema exposes delivery-command records | add chaos only if distributed handoff serialization, endpoint lease churn, or adapter-node takeover can corrupt command fact ownership | `covered-delivery-command-fact-ownership` | do not prove this with old field-copy tests, broad source scans alone, nested worker pull DTOs/`TransportPacket` payloads, `TransportDispatchEnvelope.deliveryQueueKey`, Redis value queue-key copies, fabricated unknown route/node fields, or a generic `correlation` bag |
