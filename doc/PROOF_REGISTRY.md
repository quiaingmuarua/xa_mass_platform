...
| `transport.worker-route-owner` | route-owner truth is keyed by opaque route consumer records; current SDK/starter minting is worker-group-level and outside transport runtime; one route can have multiple active consumer records, stale session tokens cannot revoke newer consumers, producer dispatch lookup uses a derived `adapterId + selectedWorkerId` owner pointer validated against the live consumer lease, and transport delivery does not decide worker capacity | transport runtime/API proof: `CanonicalWorkerGroupRouteKeyCodecTest` for the SDK/starter default worker-group route-key codec, `RedisTransportRouteOwnerStoreTest`, `InMemoryTransportRouteOwnerStoreTest`, `TransportRouteOwnerViewTest`, `InMemoryTransportDeliveryCommandHandoffTest`, `TransportDeliveryCommandBatchCodecTest`, `RedisTransportDeliveryCommandHandoffTest`, `RedisTransportDeliveryFailureChannelTest`, `TransportDeliveryServiceTest`, `RedisTransportDeliveryStoreTest`, `TransportConvergenceArchitectureGuardTest` | SDK/server representative proof: `MassApplicationDistributedTransportTest`, `ExternalWorkerApiControllerTest`, `ExternalWorkerPollingApiIntegrationTest`, `ExternalWorkerRealtimeRegistrationIntegrationTest`, plus public Java SDK `WorkerClientTest`, `PollingWorkerSessionTest`, `WebSocketWorkerSessionTest` | no dedicated trace analyzer currently; route-owner proof is runtime/transport/API behavior until trace schema exposes route-owner events | add chaos only for real distributed route-owner churn, Redis route-owner failure, or adapter process takeover beyond current deterministic proof | `covered-delivery-command-dispatch` | do not use raw source scans, field-copy tests, workerId route resolver helpers, or worker-id inspection views as route-owner proof; transport Redis must keep `workers`, `owner-shards`, `worker-routes:*`, `worker-route:*`, `routes`, `route-presence:*`, and worker runtime/admission keys out of transport truth, while the derived adapter-worker owner pointer must remain delivery-feasibility evidence only |
