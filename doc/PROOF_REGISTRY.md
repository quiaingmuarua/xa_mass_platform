...
| `ext.worker-pack-sdk-capability` | worker-pack capability groups can run as external Java SDK workers without privileged server startup or raw Java transport demos | `GeoLookupToolTest` for handler/group contract | `WorkerPackGeoLookupExternalSdkIntegrationTest` | trace-observed coverage belongs to `ext.worker-parity-public-contract` unless worker-pack adds a distinct fault/capability trace invariant | add distributed proof only when a real worker-pack capability depends on external provider timing or network churn | `covered` | do not add worker-pack raw transport demos as proof when an SDK-backed capability test covers the public worker path |
