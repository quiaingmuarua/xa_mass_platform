...
| `transport.worker-route-owner` | worker dispatch route ownership is keyed by opaque `routeKey`; current SDK/starter minting is worker-group-level and outside transport runtime; one route can have multiple active consumer records, stale session tokens cannot revoke newer consumers, dispatch lanes are route-domain plus selected consumer locality, and transport delivery does not decide worker capacity | transport runtime/API proof: `CanonicalWorkerGroupRouteKeyCodecTest` for the SDK/starter default worker-group route-key codec, `RedisTransportRouteOwnerStoreTest`, `InMemoryTransportRouteOwnerStoreTest`, `TransportRouteOwnerViewTest`, `RouteTargetedTaskDispatchSubmitterTest`, `InMemoryRouteTargetedTaskDispatchHandoffTest`, `RedisRouteTargetedTaskDispatchHandoffTest`, `RouteTargetedTaskDispatchHandoffPumpTest`, `TransportDeliveryServiceTest`, `RedisTransportDeliveryStoreTest` | SDK/server representative proof: `MassApplicationDistributedTransportTest`, `ExternalWorkerApiControllerTest`, `ExternalWorkerPollingApiIntegrationTest`, `ExternalWorkerRealtimeRegistrationIntegrationTest`, plus public Java SDK `WorkerClientTest`, `PollingWorkerSessionTest`, `WebSocketWorkerSessionTest` | no dedicated trace analyzer currently; route-owner proof is runtime/transport/API behavior until trace schema exposes route-owner events | add chaos only for real distributed route-owner churn, Redis route-owner failure, or adapter process takeover beyond current deterministic proof | `covered-route-targeted-dispatch` | do not use raw source scans, field-copy tests, or workerId route resolver helpers as route-owner proof; transport Redis must keep `workers`, `owner-shards`, `worker-routes:*`, `route-presence:*`, and worker runtime/admission keys out of transport truth |
