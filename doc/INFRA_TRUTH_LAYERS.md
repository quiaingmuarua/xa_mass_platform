...
| engine -> transport dispatch payload | runtime state | best-effort delivery-attempt handoff state: bounded admission, mailbox-scoped claim/ack, and observable known failures | bounded in-memory or Redis `TransportDeliveryCommandHandoff` carrying mailbox-targeted dispatch items | JDBC/review-row truth, a duplicate engine ready queue, or a transport-owned retry/final-recovery state machine |
| transport -> engine result / delivery-failure inboxes | runtime state | hot-path cross-JVM ingress back into engine-owned result/compensation ports | bounded Redis inboxes drained by engine process | server/API owner semantics or transport-owned lifecycle state |
...
| Redis transport dispatch handoff | post-claim assignment queue between engine and transport JVMs; delivery integration resolves the selected worker to an opaque `adapterMailboxKey`, while handoff-private mailbox consumer lease evidence owns ready/inflight/ack mechanics | runtime-state delivery-executor handoff, not ready queue ownership, not worker routing truth, and not task lifecycle truth |
...
| Redis transport result / delivery-failure inboxes | transport-to-engine runtime ingress | bounded cross-JVM channels drained into engine-owned result ingest and compensation ports, not server endpoints |
