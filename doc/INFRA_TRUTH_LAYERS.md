...
| engine -> transport dispatch payload | runtime state | best-effort delivery-attempt handoff state: bounded admission, mailbox-scoped claim/ack, and observable known failures | bounded in-memory or Redis `TransportDispatchHandoff` carrying mailbox-targeted dispatch items | JDBC/review-row truth, a duplicate engine ready queue, or a transport-owned retry/final-recovery state machine |
