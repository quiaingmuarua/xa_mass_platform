...
| runtime result apply | runtime state | active lease, retry budget consumption, finality status, counters, and idempotent final-result lookup are hot-path truth | `xa-mass-task-runtime` `TaskRuntimeConvergencePort#applyResult(RuntimeResultFact)` | review rows or transport envelope metadata |
