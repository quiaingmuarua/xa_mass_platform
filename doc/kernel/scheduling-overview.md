...
| Business usage observation, such as successful assignment | Server's fixed observation handlers and the scenario's pure projection | Selected Platform Properties, for example App Checks assignment-window observations | The projection is best-effort observation and does not reserve execution capacity. |
