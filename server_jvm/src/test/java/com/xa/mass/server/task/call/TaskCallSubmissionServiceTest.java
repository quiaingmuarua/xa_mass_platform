...
class TaskCallSubmissionServiceTest {
    @Test void itemFunctionCanDifferFromTheTasksSupplyRule() {
        ...
        var descriptor = new TaskRuntime.TaskDescriptor("task", "test-project", "group", TaskRuntime.TaskIdleDisposition.PARK_WHEN_IDLE, Map.of("priority", "0", "maxRetryTimes", "3"), List.of(new RefillTarget("country", new com.xa.mass.kernel.assignment.EligibilityQuery(Map.of()), 100)));
    ...
    @Test void matchingFailureRejectsCallWithoutSubmission() {
        ...
        when(catalog.loadTaskAllocationDescriptors(List.of("task"))).thenReturn(Map.of(
                "task", new TaskRuntime.TaskDescriptor("task", "test-project", "group", TaskRuntime.TaskIdleDisposition.PARK_WHEN_IDLE, Map.of("priority", "0", "maxRetryTimes", "3"), java.util.List.of(new com.xa.mass.kernel.assignment.RefillTarget("any", new com.xa.mass.kernel.assignment.EligibilityQuery(java.util.Map.of()), 100)))));
    ...
    @Test void matchingAdmissionValidatesOverwrittenInputsAndPersistsTheSelectorWithoutSelection() {
        ...
        var descriptor = new TaskRuntime.TaskDescriptor("task", "test-project", "group", TaskRuntime.TaskIdleDisposition.PARK_WHEN_IDLE, Map.of("priority", "0", "maxRetryTimes", "3"), java.util.List.of(new com.xa.mass.kernel.assignment.RefillTarget("any", new com.xa.mass.kernel.assignment.EligibilityQuery(java.util.Map.of()), 100)));
        when(catalog.loadTaskAllocationDescriptors(List.of("task"))).thenReturn(Map.of("task", descriptor));
        var service = new TaskCallSubmissionService(submission, catalog, new TaskItemMapper(), matching);
        ...
        var bad = new TaskItemRequest("id", "event", Map.of(), 5, 1000L, new WorkerQuery("worker.country", List.of("cn")));
        var good = new TaskItemRequest("id", "event", Map.of(), 5, 1000L, new WorkerQuery("worker.country", List.of("CN")));
    ...
    void invalidJavaInputsIncludingOverwrittenDuplicatesNeverReachAnOwner() {
        ...
        var invalid = List.of(
                ...
                new TaskItemRequest("id", " ", Map.of(), 5, 1000L, new WorkerQuery("worker.any", Map.of())),
                new TaskItemRequest("id", "event", null, 5, 1000L, new WorkerQuery("worker.any", Map.of())),
                new TaskItemRequest("id", "event", Map.of(), 11, 1000L, new WorkerQuery("worker.any", Map.of())),
                new TaskItemRequest("id", "event", Map.of(), 5, 0L, new WorkerQuery("worker.any", Map.of())),
                new TaskItemRequest("id", "event", Map.of(), 5, 1000L, null));
        assertThatThrownBy(() -> new WorkerQuery("worker.any", new Object()))
    ...
    void unsupportedRuleParametersAreRejectedByMatchingEvenWhenOverwritten() {
        ...
        for (Map<String, ?> expression : List.of(
            ...
            var selector = new WorkerQuery("worker.any", expression);
            doThrow(new IllegalArgumentException("unsupported")).when(matching).normalizeQuery("group",selector);
            var rejected = new TaskItemRequest("id", "event", Map.of(), 5, 1000L, new WorkerQuery("worker.any", new HashMap<>(expression)));
