...
@Name("xa.mass.TaskResult") @Label("Task call stage") @Category("XA Mass")
@Enabled(false) @StackTrace(false)
final class ResultStageEvent extends Event {
    private static final EventType TYPE = EventType.getEventType(ResultStageEvent.class);
    public String stage;
    public String key;
    ...
    private static void emit(long started, long ended, String stage, String key, int size, int count, boolean failed) {
        var event = new ResultStageEvent();
        event.stage = stage;
        event.key = key;
