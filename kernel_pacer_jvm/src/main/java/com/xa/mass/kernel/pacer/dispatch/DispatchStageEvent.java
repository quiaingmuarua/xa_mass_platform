...
@Name("xa.mass.TaskDispatch") @Label("Task call stage") @Category("XA Mass")
@Enabled(false) @StackTrace(false)
final class DispatchStageEvent extends Event {
    private static final EventType TYPE = EventType.getEventType(DispatchStageEvent.class);
    public String stage;
    public String key;
    ...
    private static void emit(long started, long ended, String stage, String key, int size, int count, boolean failed) {
        var event = new DispatchStageEvent();
        event.stage = stage;
        event.key = key;
