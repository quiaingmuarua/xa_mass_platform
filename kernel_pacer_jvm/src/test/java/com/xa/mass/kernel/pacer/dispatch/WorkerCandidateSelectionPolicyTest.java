...
class WorkerCandidateSelectionPolicyTest {
    ...
    @Test void everySelectorConsumesSharedHeldStockAndReadsAddressesInOneBatch() {
        ...
        var selected=policy.takeCandidates(query,"g",items,new HashSet<>());
    ...
    @Test void unavailableBindingNeverFallsBackAndWrongGroupIsNotAssigned() {
        ...
        verifyNoInteractions(query,catalog);
        when(query.take(anyMap())).thenReturn(Map.of(any,List.of(new HeldCandidate("w",1,6000))));
        ...
        assertTrue(policy.takeCandidates(query,"g",Map.of("m",any),round).isEmpty());
    ...
    @Test void addressFailureConsumesTheCandidateAndCannotReviveItsFence() {
        ...
        var round=new HashSet<String>();
        assertThrows(IllegalStateException.class,()->policy.takeCandidates(query,"g",Map.of("m",any),round));
        assertTrue(policy.takeCandidates(query,"g",Map.of("next",any),round).isEmpty());
        verify(catalog,times(1)).getWorkerDescriptors(anyList());
    }
}
