...
class WorkerCandidateSelectionPolicyTest {
    ...
    @Test void addressFailureConsumesTheCandidateAndCannotReviveItsFence() {
        when(matching.take("g","worker.default",Map.of("m",any))).thenReturn(Map.of("m",new WorkerCandidate("w",1)));
        when(matching.take("g","worker.default",Map.of("next",any))).thenReturn(Map.of("next",new WorkerCandidate("w",1)));
