...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `TaskRuntimeAppendPort#appendBatch(AppendBatchCommand)` | old callers migrated; old port/DTO deleted from core main | `ids`, `ready`, `tasks`, `dirty` | `TaskRuntimeWorkPort#appendBacklog(taskId, frames, maxAppendBatchSize)` | Serving append callers use `appendBacklog`. | Newly accepted Redis work writes only `<tr>:task:{taskId}:backlog`; append does not score-enroll. | Keep guards preventing old append port/DTO re-entry. | Closed for core/starter/engine/Redis serving. |
