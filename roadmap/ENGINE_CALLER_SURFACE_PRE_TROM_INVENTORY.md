...
| `TaskCommandPort`, `TaskQueryPort`, deleted `TaskCommandService` / `TaskQueryService` names, `TaskEventService`, `WorkerControlRuntime`, `TaskStageEvidenceService`, `TaskResultRuntime` imports from `MassSdkApplication` | `MassSdkApplication` before ECSP | forbidden service/runtime owner import | engine, worker-runtime, task-result runtime | Not temporary. Replaced by behavior methods on `MassApplication` and `MassEngine`. | `xa-mass-engine-starter` containment only | Must remain absent from embedded SDK main source. | ECSP-2/4 |
| `com.xa.mass.engine.model.TaskDefinitionPatch`, `TaskCommandOutcome`, `TaskAppendOutcome`, `TaskStateValidationResult`, `TaskStateResolutionResult` | `MassApplication`, `MassSdkApplication`, diagnostics operations | temporary value-contract exception | `xa-mass-engine` today | Engine command shapes are now bounded outcomes; deleted `TaskAppendReceipt` / `TaskResumeResult` are no longer starter-facing engine models. Remaining value shapes still need TROM/public-contract classification. | future task-runtime, engine-shell, or public runtime contract module | Reclassify remaining exceptions during TROM-0 module split inventory. | ECSP-0B/TROM-0 |
...
| Task shell and item operations | `MassSdkApplication`, diagnostics operations | create shell, load/list tasks, append items, patch task definition, approve/reject/block/pause/resume/cancel/terminate/seal | task id, stable request DTOs, `TaskDefinitionPatch`, item maps | bounded `TaskCommandOutcome` / `TaskAppendOutcome` adapted into SDK-owned receipts/snapshots and booleans | engine task shell/command/query owners | SDK core API needs task behavior without service getters; engine command port no longer returns mutable `Task` or engine receipt/detail DTOs. | task command/query service getters | `MassSdkTest` |
| Task runtime diagnostic operations | `MassSdkApplication`, diagnostics operations, server E2E support | read owner-backed active leases and aggregate runtime stats for a selected task | task id | `TaskWorkStats`, active lease records | task-runtime serving lane behind engine-starter | SDK/server diagnostics need runtime snapshots without reading old `TaskWorkRuntime` stores or `EngineConfig` getters. | old task work runtime stats/active-lease getter path | `ExternalWorkerPollingApiIntegrationTest`, `RedisRuntimeExternalWorkerPollingApiIntegrationTest`, guard tests |
