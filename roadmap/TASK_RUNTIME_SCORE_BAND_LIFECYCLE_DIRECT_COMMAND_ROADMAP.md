...
- `pause` is a no-argument lifecycle event. Runtime maps it to its configured
  default future timestamp; callers do not pass pause duration or pause-until
...
- TRLC-5 retry rescore is narrowed: `TaskRuntimeServingLane` uses the
  result-owned `retryAtMillis` to write the next task score and no longer
  reloads the shell task or calls `updateSchedulerEligibility(...)` for retry
  rescore.
...
embedded SDK/server validated command
  ...
  -> lifecycle/score-owner/maintenance commands update runtime reason + task:score:{laneKey}
...
task-runtime maintenance loop or explicit runtime repair command
  -> scan score-visible maintenance candidates / retry due / active leases
  -> update runtime reason/score by runtime policy
...
transport/worker result ingest
  ...
  -> update work truth + task reason/score
...
- Task score is written only by lifecycle/score-owner/maintenance owners:
  approve/resume/block/pause/reject/cancel/terminate, retry promotion, lease
  repair, result/finality, close/discard, or an explicit score-owner rescore
  action.
...
| scheduled future work / delayed pause | timestamp score in the schedulable-time band, `score > now`; this is a time-bounded delay and will become due automatically |
| non-schedulable but non-terminal | positive enum score outside the timestamp band, such as created/unapproved, manual hold, blocked, active-maintenance-only, or policy-held |
...
| paused / delayed | optional owner-local delay reason | future timestamp score chosen by runtime default pause policy; still in the schedulable-time band | no dispatch until due; resume rewrites timestamp to due time |
...
maps a pause event to a future schedulable timestamp using its default pause
window, for example `now + defaultPauseMillis`; no caller-provided pause time is
accepted in this roadmap. The task will auto-resume when that timestamp becomes
due. Manual hold or indefinite operator stop uses a positive non-schedulable
enum such as manual blocked. Terminal must not be represented as another
removable non-schedulable enum during lifecycle convergence.
...
| pause event | move score to a runtime-chosen future schedulable timestamp; no caller-provided pause time and no separate paused parked state |
...
- Replace the current `TaskScoreV1` band contract before implementing the
  command state machine. Current `PARKED_PAUSED`, `PARKED_BLOCKED`, and
  negative parked semantics are incompatible with this roadmap.
...
- Append writes backlog only. It never writes task score, even when the task
  ...
  score-owner lifecycle, maintenance, result, and close/discard paths.
...
- `TaskRuntimeServingLaneTest` proves the current serving append -> claim ->
  ...
  maintenance dispatch visibility.
...
- `TaskRuntimeServingLaneTest#retryableLeaseExpiryEmitsRetryResetTrace`
  proves retryable lease expiry writes the next runtime score from
  result-owned retry timing instead of shell status.
