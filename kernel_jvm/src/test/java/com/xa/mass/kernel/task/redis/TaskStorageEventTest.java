...
class TaskStorageEventTest {
    @TempDir Path temporary;

    @Test void disabledPathIsInertAndSamplingHasTheSharedFixedVector() {
        assertEquals(0, TaskStorageEvent.start());
        assertDoesNotThrow(() -> TaskStorageEvent.items(0, "SUBMISSION", null, null, 0, false));
        assertEquals("40a73f95e3436b2db191dded2ae14e8764ead7378784e6d80c8b933320f16f0a",
                TaskStorageEvent.sampleKey("task", "item-87"));
        assertNull(TaskStorageEvent.sampleKey("task", "item-0"));
    }
    ...
    @Test void recordingExportsOneAggregateAndOneSampleWithoutPrivateInputs() throws Exception {
        ...
        try (var recording = new Recording()) {
            recording.enable(TaskStorageEvent.class);
            recording.start();
            long started = TaskStorageEvent.start();
            assertNotEquals(0, started);
            TaskStorageEvent.items(started, "SUBMISSION", "task", List.of("item-87"), 1, false);
            assertDoesNotThrow(() -> TaskStorageEvent.items(started, "SUBMISSION", null, null, 0, false));
            recording.stop();
            recording.dump(path);
        }
