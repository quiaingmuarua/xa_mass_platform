...
| [Worker Call Performance](integrations/worker-call-performance/README.md) | `python integrations/worker-call-performance/run_worker_call_performance.py --suite nightly` (seven aligned RPC cases plus original mixed witness); `--suite rpc-diagnosis --repetitions 3` and JFR are manual, as are historical Task/Direct suites | Ubuntu 24.04, 4 logical CPUs, Docker Redis 7.4.10, Java 21 |
