...
| Runtime Boundary | Python Kernel, Java Server, Polling, WebSocket, and Socket close real Task paths; WebSocket also proves pause-gated custom/default Worker controls plus Adapter connection observe/close/reconnect through the unified APIs | Redis 7 and Python Kernel | `./gradlew :server_jvm:runtimeBoundaryIntegrationTest` |
...
The same proof pauses a real WebSocket Worker, invokes a custom SYSTEM handler
and the default probe/properties/events handlers, observes and closes its current
Adapter Channel, proves transparent reconnect, and resumes scheduling. It
proves the current single Server / unified Adapter HTTP path; it is not
evidence for a distributed CONTROL_ONLY mailbox or cross-instance waiter
correlation.
