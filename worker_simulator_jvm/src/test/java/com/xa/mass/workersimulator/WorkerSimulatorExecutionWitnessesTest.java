...
class WorkerSimulatorExecutionWitnessesTest {
    @Test void actualReplicaClosuresRemainDistinctEvenWithTheSameToken() throws Exception {
        var journal = new WorkerSimulatorExecutionWitnesses();
    ...
    @Test void overflowIsStickyAndNeverEvictsEvidence() throws Exception {
        ...
        for (int i = 0; i < WorkerSimulatorExecutionWitnesses.CAPACITY / 2; i++)
        ...
        assertThat(journal.read(WorkerSimulatorExecutionWitnesses.CAPACITY - 1, 1).get("nextCursor"))
                .isEqualTo((long) WorkerSimulatorExecutionWitnesses.CAPACITY);
