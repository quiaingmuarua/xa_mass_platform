...
class ListeningRegistryTest {
    @Test void inputThatLookedUpAnOldAddressCannotReachItsNextOccupant() throws Exception {
        var oldProperties = new java.util.concurrent.atomic.AtomicReference<>(Map.of("phone", "old", "country", "CN"));
        ...
        synchronized (source) {
            ...
            while (incoming.getState() != Thread.State.BLOCKED && System.nanoTime() < end) Thread.sleep(1);
            assertThat(incoming.getState()).isEqualTo(Thread.State.BLOCKED);
            var replacement = Map.of("phone", "changed", "country", "CN");
            registry.updateProperties(source, replacement, () -> oldProperties.set(replacement));
            var reused = Map.of("phone", "old", "country", "CN");
            registry.updateProperties(target, reused, () -> nextProperties.set(reused));
            registry.listen(target, input("new-occupant", 100, "CODE"), reporter);
        }
        incoming.join(2_000);
    ...
    @Test void stoppedRuntimeIsCleanedWithoutTreatingTransparentReconnectAsAStop() {
        ...
        var sim = registry.addSim("demo-sim", "US-0", () -> Map.of("phone", "102", "country", "CN"), () -> "worker-3", runtime::get, () -> true);
    ...
    final ListeningRegistry.Sim first = registry.addSim("demo-sim", "CN-0", () -> Map.of("phone", "100", "country", "CN"), () -> "worker-1", () -> "RUNNING", () -> true);
    final ListeningRegistry.Sim second = registry.addSim("demo-sim", "CN-1", () -> Map.of("phone", "101", "country", "CN"), () -> "worker-2", () -> "RUNNING", () -> true);
    ...
    @Test void boundedRecordsNeverEvictDedupAndExistingListenersContinue() {
        ...
        var sim = bounded.addSim("demo-sim", "CN-0", () -> Map.of("phone", "100", "country", "CN"), () -> "w", () -> "RUNNING", () -> true);
