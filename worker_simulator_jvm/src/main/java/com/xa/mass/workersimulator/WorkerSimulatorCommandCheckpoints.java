...
final class WorkerSimulatorCommandCheckpoints implements AutoCloseable {

    static final long MAX_HOLD_MILLIS = 120_000L;

    private final Map<WorkerSimulatorCoordinate, Gate> gatesByWorker =
            new LinkedHashMap<>();
    ...
    synchronized Snapshot snapshot(WorkerSimulatorCoordinate worker) {
        Objects.requireNonNull(worker, "worker");
    ...
    void release(WorkerSimulatorCoordinate worker) {
        Gate gate;
        synchronized (this) {
            Objects.requireNonNull(worker, "worker");
    ...
    private static final class Gate {
        ...
        private final WorkerSimulatorCoordinate worker;
