...
final class WorkerSimulatorScheduledStops implements AutoCloseable {

    static final long MAX_DELAY_MILLIS = Duration.ofDays(1).toMillis();

    private static final System.Logger LOGGER = System.getLogger(
            WorkerSimulatorScheduledStops.class.getName()
    );
    ...
    WorkerSimulatorScheduledStops(WorkerSimulator workers) {
        this.workers = Objects.requireNonNull(workers, "workers");
        scheduler = Executors.newSingleThreadScheduledExecutor(task -> {
            Thread thread = new Thread(
                    task,
                    "worker-simulator-scheduled-stops"
            );
        ...
        try {
            if (!scheduler.awaitTermination(5, TimeUnit.SECONDS)) {
                LOGGER.log(
                        ...
                        "Worker Simulator scheduled-stop owner did not close"
                );
            }
    ...
    private void fire(WorkerCoordinate coordinate) {
        ...
        try {
            ...
            LOGGER.log(
                    System.Logger.Level.WARNING,
                    "Scheduled Worker Simulator stop failed",
                    error
            );
        }
    }
    ...
    private void ensureOpen() {
        if (closed) {
            throw new IllegalStateException(
                    "Worker Simulator scheduled-stop owner is closed"
            );
        }
    }
