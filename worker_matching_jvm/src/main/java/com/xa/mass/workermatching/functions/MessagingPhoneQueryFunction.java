...
public final class MessagingPhoneQueryFunction implements QueryFunction {
    ...
    public MessagingPhoneQueryFunction(PhoneIndex phones,
    ...
    @Override public Map<String, WorkerCandidate> apply(String workerGroupId, Map<String, Object> inputsByMessageId) {
        ...
        inputsByMessageId.values().forEach(input -> counts.merge((String) ((Map<?, ?>) input).get("phone"), 1, Integer::sum));
        var found = phones.lookup(workerGroupId, counts);
        var ids = new LinkedHashSet<String>();
        found.values().forEach(ids::addAll);
        ...
        inputsByMessageId.forEach((messageId, input) -> {
            ...
            for (String workerId : found.get(phone)) {
                ...
                if (country == null || !phone.equals(workerFacts.get("phone"))
                        || (countries != null && !countries.contains(country)) || !used.add(workerId)) continue;
